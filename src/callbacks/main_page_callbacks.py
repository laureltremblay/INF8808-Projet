...
# TODO: Move because that code is shit af <3 (but it still works...)
def register_main_page_callbacks(app, data_df):
    ...
    def update_figure_and_main_choices(scatter_clicks, 
                                       scatter_class, 
                                       heatmap_clicks, 
                                       main_choice, 
                                       selected_main, 
        ...
        if selected_main:
            if main_choice == 'team':
                filtered_df = data_df[data_df['teamCode'].isin([selected_main] if isinstance(selected_main, str) else selected_main)]
            elif main_choice == 'player':
                filtered_df = data_df[data_df['shooterName'].isin([selected_main] if isinstance(selected_main, str) else selected_main)]
                color_var = 'shooterName'
            elif main_choice == 'position':
                filtered_df = data_df[data_df['playerPositionThatDidEvent'].isin([selected_main] if isinstance(selected_main, str) else selected_main)]
                color_var = 'playerPositionThatDidEvent'
            elif main_choice == 'goalie':
                filtered_df = data_df[data_df['goalieNameForShot'].isin([selected_main] if isinstance(selected_main, str) else selected_main)]
        ...
        filtered_df = filtered_df[filtered_df['shotType'] == shot_type_choice] if shot_type_choice != 'all' else filtered_df

        # Filter by period choice
        filtered_df = filtered_df[filtered_df['period'] == int(period_choice)] if period_choice != 'all' else filtered_df
        ...
        if n_play_choice == 'even_strength':
            filtered_df = filtered_df[(filtered_df['shootingTeamForwardsOnIce'] + filtered_df['shootingTeamDefencemenOnIce']) == (filtered_df['defendingTeamForwardsOnIce'] + filtered_df['defendingTeamDefencemenOnIce'])]
        ...
        filtered_df = filtered_df[filtered_df['defendingTeamAverageTimeOnIce'] >= toi_choice]
        ...
        if ctx.triggered_id == 'heatmap-button':
            ...
            return get_scatter_figure(filtered_df, color_var), 'header-button heatmap-button', 'header-button scatter-button active'
        else: 
            return get_heatmap_figure(filtered_df), 'header-button heatmap-button active', 'header-button scatter-button'
    ...
    def toggle_main_choice_dropdown(selected_value):
        if selected_value == 'team':
            ...
            options = [{'label': player, 'value': player} for player in sorted(data_df['shooterName'].unique(), key=lambda name: name.split()[-1])]
            return 'Choisir un joueur', options, False
        
        elif selected_value == 'position':
            options = [{'label': pos, 'value': pos} for pos in sorted(data_df['playerPositionThatDidEvent'].unique())]
            return 'Choisir une position', options, False
        
        elif selected_value == 'goalie':
            options = [{'label': goalie, 'value': goalie} for goalie in sorted(data_df['goalieNameForShot'].unique(), key=lambda name: name.split()[-1])]
